...
class LeftListItem {
  ...
  get unreadMessageCount() {
    const count =
      this.channel.unreadMessageCount > 9
        ? "+9"
        : this.channel.unreadMessageCount.toString();
  ...
  _createElement(handler) {
    ...
    if (this.channel.isOpenChannel()) {
      ...
      const itemTopTitle = createDivEl({
      ...
      const itemTop = createDivEl({ className: styles["item-top"] });
      const itemTopCount = createDivEl({
      ...
      const itemBottomMessage = createDivEl({
      ...
      const itemBottomTime = createDivEl({
        className: styles["item-time"],
        content: this.lastMessageTimeText
      });
      setDataInElement(
        itemBottomTime,
        KEY_MESSAGE_LAST_TIME,
  ...
  static updateUnreadCount() {
    const items = ChatLeftMenu.getInstance().groupChannelList.getElementsByClassName(
      styles["item-message-unread"]
    );
