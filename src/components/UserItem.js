...
class UserItem {
  ...
  _createElement(handler) {
    ...
    const profile = createDivEl({
      className: styles["user-profile"],
      background: this.profileUrl
    });
    ...
    const isOnline = createDivEl({
      className: this.isOnline
        ? [styles["user-online"], styles.active]
        : styles["user-online"]
    });
    ...
    const lastSeenTime = createDivEl({
      className: styles["user-time"],
      content: this.lastSeenTimeString
    });
    ...
    item.addEventListener("click", () => {
      toggleClass(
        item.querySelector(`.${UserItem.selectIconClassName}`),
        styles.active
      );
      if (handler) handler();
    });
